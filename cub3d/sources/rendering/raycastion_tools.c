...
/*   Updated: 2025/07/27 09:05:18 by eel-garo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	game->rays[i].is_ray_facing_down = ray_angle > 0 && ray_angle < PI;
	game->rays[i].is_ray_facing_up = !game->rays[i].is_ray_facing_down;
	game->rays[i].is_ray_facing_left = ray_angle > (PI / 2)
		&& ray_angle < (1.5 * PI);
...
bool	hit_wall(t_game *game, float x, float y)
{
	...
	if (x < 0 || x >= (game->map.width * TILE_SIZE)
		|| y < 0 || y >= (game->map.height * TILE_SIZE))
		return (true);
